...
- **Debug logs go through `utils/AppLog.kt`** (`AppLog.d(...)` / `AppLog.w(...)`) — it centralizes the `BuildConfig.DEBUG` gate and the `"roy93~"` tag so a call site can't forget either. `doc/Security.md` H-01 flags raw logging of health data, weight, height, BMI, and GAID as a GDPR/PII issue. `Log.e(...)` calls stay ungated (generic error diagnostics, not PII) and component-scoped tags (e.g. `"BaseActivity"`, `"MainAct"`) stay as direct `if (BuildConfig.DEBUG) Log.w(...)` calls for log filtering — only the shared `"roy93~"` debug/warn tag was consolidated into `AppLog`.
